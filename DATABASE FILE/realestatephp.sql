-- Active: 1726853384418@@127.0.0.1@3306@realestatephp
-- phpMyAdmin SQL Dump
-- version 4.6.5.2
...
FOR EACH ROW 
BEGIN
    INSERT INTO property_audit (property_id, action_type, action_date, user_id)
    VALUES (NEW.pid, 'INSERT', NOW(), NEW.uid);
END//
DELIMITER ;

DELIMITER //
CREATE FUNCTION get_user_property_count(user_id INT) 
RETURNS INT
DETERMINISTIC
BEGIN
...
DELIMITER //
CREATE PROCEDURE get_properties_by_city(IN city_name VARCHAR(200))
BEGIN
    SELECT p.*, u.uname as owner_name
    FROM property p 
